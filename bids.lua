...
retep_bids = retep:NewModule("retep_bids", "AceDB-2.0", "AceEvent-2.0")

function retep_bids:OnEnable()
  if not T:IsRegistered("retep_bids") then
    T:Register("retep_bids",
      "children", function()
        T:SetTitle(L["retroll bids"])
      ...
      "menu", function()
        D:AddLine(
          ...
          "func", function() retep_bids:Refresh() end
        )
      end      
    )
  end
...
function retep_bids:OnDisable()
  T:Close("retep_bids")
end

function retep_bids:Refresh()
  T:Refresh("retep_bids")
end

function retep_bids:setHideScript()
  local i = 1
  local tablet = getglobal(string.format("Tablet20DetachedFrame%d",i))
  while (tablet) and i<100 do
    if tablet.owner ~= nil and tablet.owner == "retep_bids" then
...
function retep_bids:Top()
  if T:IsRegistered("retep_bids") and (T.registry.retep_bids.tooltip) then
    T.registry.retep_bids.tooltip.scroll=0
  end  
end

function retep_bids:Toggle(forceShow)
  self:Top()
  if T:IsAttached("retep_bids") then
    T:Detach("retep_bids") -- show
    if (T:IsLocked("retep_bids")) then
      T:ToggleLocked("retep_bids")
    end
    self:setHideScript()
  else
    if (forceShow) then
      ...
      T:Attach("retep_bids") -- hide
    end
  end  
end

function retep_bids:announceWinnerMS(name, pr)
  retep:widestAudience(string.format(L["Winning Mainspec Bid: %s (%.03f PR)"],name,pr))
end

function retep_bids:announceWinnerOS(name, pr)
  retep:widestAudience(string.format(L["Winning Offspec Bid: %s (%.03f PR)"],name,pr))
end

function retep_bids:countdownCounter()
  ...
  if GetNumRaidMembers()>0 and self._counter > 0 then
    ...
    retep:widestAudience(tostring(self._counter))
    --SendChatMessage(tostring(self._counter),"RAID")
    self:Refresh()
  end
end

function retep_bids:countdownFinish(reset)
  if self:IsEventScheduled("retrollBidCountdown") then
    self:CancelScheduledEvent("retrollBidCountdown")
  end
  self._counter = 6
  if (reset) then
...
function retep_bids:bidCountdown()
  self:countdownFinish(true)
  self:ScheduleRepeatingEvent("retrollBidCountdown",self.countdownCounter,1,self)
  self:ScheduleEvent("retrollBidCountdownFinish",self.countdownFinish,6,self)
end

local pr_sorter_bids = function(a,b)
  if retep_minep > 0 then
    local a_over = a[3]-retep_minep >= 0
    local b_over = b[3]-retep_minep >= 0
...
function retep_bids:BuildBidsTable()
  -- {name,class,ep,gp,ep/gp[,main]}
  table.sort(retep.bids_main, pr_sorter_bids)
  table.sort(retep.bids_off, pr_sorter_bids)
  return retep.bids_main, retep.bids_off
end

function retep_bids:OnTooltipUpdate()
  if not (retep.bid_item and retep.bid_item.link) then return end
  local link = retep.bid_item.link
  local itemName = retep.bid_item.name
  local price = retep_prices:GetPrice(link,retep_progress)
  local offspec
  if not price then 
    price = "<n/a>"
    offspec = "<n/a>"
  else
    offspec = math.floor(price*retep_discount)
  end
